...
SECONDARY_ENABLE_TOKEN=$(vault write sys/replication/dr/primary/secondary-token id="dr-secondary" -format=json | jq -r '.wrap_info.token')
echo $SECONDARY_ENABLE_TOKEN

### Not Scripted ###
# - Sign in to dr
####################

vault write sys/replication/dr/secondary/enable token=$SECONDARY_ENABLE_TOKEN

vault read sys/replication/dr/status -format=json | jq
