...
vault write pki/roles/v1 allow_any_name=true basic_constraints_valid_for_non_ca=true && vault write -f pki/issue/v1 common_name=max.com ttl=30d -format=json | jq -r '.data.certificate' | openssl x509 -inform pem -text -noout



vault write -f pki/issue/v1 common_name=max.com ttl=30d


openssl genpkey -algorithm RSA -out private.key
openssl req -new -key private.key -out certificate.csr -subj "/CN=YourCommonName" -addext "basicConstraints=CA:FALSE" -addext "subjectAltName=DNS:example.com
req certificate.csr

vault write pki/sign-verbatim/v1 csr=@certificate.csr ttl=30d -format=json | jq -r '.data.certificate' | openssl x509 -inform pem -text -noout

vault write pki/sign-verbatim/v1 csr=@certificate.csr ttl=30d
